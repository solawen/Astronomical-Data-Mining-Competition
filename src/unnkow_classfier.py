...
if __name__ == '__main__':
    df = pd.read_csv('../data/first_train_index_20180131.csv')
    qso = df[df['type']=='qso']
    galaxy = df[df['type']=='galaxy']
    star = df[df['type']=='star']
    unknown = df[df['type']=='unknown']
    frames = [star,unknown,qso,galaxy]
    cw = {0:1,1:14}
    ...
    #optimizer = Adam(lr=lr_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay=1e-6)
    ...
    #------------------Saving---------------------
    Version = '0.0.1'
    model.save('two_classfier_unknown_model_ver'+Version+'.h5')
    np.save('mean_X_two_classfier_unknown_model_ver'+Version+'.npy',mean_X)
    np.save('std_X_two_classfier_unknown_model_ver'+Version+'.npy',std_X)
    #---------------------------------------------